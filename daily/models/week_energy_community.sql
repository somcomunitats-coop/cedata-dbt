...
select  md5(concat(date_trunc('week', ts) ,'-', c.community_id))::UUID as id,
date_trunc('week', ts) as ts, c.community_id as community_id
...
where date_trunc('week', ts)>=CURRENT_DATE-5
group by c.community_id,  date_trunc('week', ts)
