...
select d.data, hsr.company_id, sum(hm.amount_untaxed) as sr_amount_untaxed
    , sum(case when hst.is_voluntary then hm.amount_untaxed end) as sr_amount_untaxed_voluntary
    , sum(case when not hst.is_voluntary then hm.amount_untaxed end) as sr_amount_untaxed_mandatory
