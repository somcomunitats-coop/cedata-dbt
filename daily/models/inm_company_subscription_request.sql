...
select d.data, hsr.company_id, sum(hm.amount_untaxed) as sr_amount_untaxed
    , sum(case when hsr.subscription_mode='voluntary' then hm.amount_untaxed end) as sr_amount_untaxed_voluntary
    , sum(case when not hsr.subscription_mode='voluntary' then hm.amount_untaxed end) as sr_amount_untaxed_mandatory
